...
{
    const harryRoommates = new Dorraymon
    ...
    const isRoommate = function (wizard) {
        if (harryRoommates.indexOf(wizard) != -1) {
            const yes = `Of course, ${wizard} is a Harry's roommate`
            return yes
        } else {
            const no = `Nope, ${wizard} is not a Harry's roommate`
            return no
        }
    }
...
{
    const gryffindors = new Dorraymon
    gryffindors[0] = 'Harry Potter'
    gryffindors.length = 1

    const theSortingHat = function (gryffindors, wizard) {
